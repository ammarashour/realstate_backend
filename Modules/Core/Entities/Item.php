...
class Item extends PsModel
{
    ...
    public function scopeLocationFilterWithLatLng($query, $filterMap)
    {
        ...
        if (array_key_exists($key, $filterMap)) {
            ...
            $latLngInfo = $filterMap[$key];

            if (
                array_key_exists($keyLat, $latLngInfo)
                && array_key_exists($keyLng, $latLngInfo)
                && array_key_exists($keyMiles, $latLngInfo)
            ) {
                $lat = $latLngInfo[$keyLat];
                $lng = $latLngInfo[$keyLng];
                $miles = $latLngInfo[$keyMiles];
